...
export default function PartidoScreen({ navigation }) {
  const [modalVisible, setModalVisible] = useState(false);
  const [fadeAnim] = useState(new Animated.Value(0)); // Animación para el desvanecimiento de la navbar

  useEffect(() => {
    // Inicia la animación cuando se monta el componente
    Animated.timing(fadeAnim, {
      toValue: 1, // El valor final de la animación (totalmente visible)
